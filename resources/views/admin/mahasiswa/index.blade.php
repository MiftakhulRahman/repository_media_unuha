...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-neutral-500 md:ms-2">Kelola Mahasiswa</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex justify-between items-center mt-4 mb-6">
        <h1 class="text-2xl font-bold text-neutral-800">Daftar Mahasiswa</h1>
        <a href="{{ route('admin.mahasiswa.create') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-primary-600 border border-transparent rounded-lg shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
            ...
            <table class="w-full text-sm text-left text-neutral-600">
                <thead class="text-xs text-neutral-700 uppercase bg-neutral-100/80">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 font-semibold">Nama Lengkap</th>
                        <th scope="col" class="px-6 py-3 font-semibold">Program Studi</th>
                ...
                <tbody class="divide-y divide-neutral-200">
                    @forelse ($mahasiswas as $mahasiswa)
                        <tr class="hover:bg-neutral-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 font-semibold text-neutral-800">{{ $mahasiswa->nim }}</td>
                            <td class="px-6 py-4 font-medium text-neutral-900 whitespace-nowrap">{{ $mahasiswa->nama_lengkap }}</td>
                            <td class="px-6 py-4">{{ $mahasiswa->prodi->nama_prodi ?? 'N/A' }}</td>
                            <td class="px-6 py-4 text-center">{{ $mahasiswa->media_count }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center items-center space-x-4">
                                    <a href="{{ route('admin.mahasiswa.edit', $mahasiswa) }}" title="Edit" class="text-primary-600 hover:text-primary-800 transition-colors">
                                    ...
                                    <form action="{{ route('admin.mahasiswa.destroy', $mahasiswa) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" data-delete-form
                                                data-delete-message="PERINGATAN: Menghapus mahasiswa <strong>{{ $mahasiswa->nama_lengkap }}</strong> akan menghapus SEMUA media yang terkait. Lanjutkan?"
