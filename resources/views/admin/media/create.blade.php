...
@section('page-title', 'Tambah Media Baru')
    ...
    <form action="{{ route('admin.media.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex items-center space-x-3">
                ...
                <button type="submit" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-primary-600 border border-transparent rounded-lg shadow-sm hover:bg-primary-700">Simpan</button>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 bg-white p-8 rounded-xl border border-neutral-200/80 shadow-sm">
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" id="judul" name="judul" value="{{ old('judul') }}" required class="mt-1 block w-full px-4 py-2.5 border border-neutral-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">
                    </div>
                    <div>
                        <label for="deskripsi" class="block text-sm font-medium text-neutral-700">Deskripsi</label>
                        <textarea name="deskripsi" id="deskripsi" rows="8" required class="mt-1 block w-full px-4 py-2.5 border border-neutral-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">{{ old('deskripsi') }}</textarea>
                    </div>
                     <div>
                        <label for="judul_penelitian" class="block text-sm font-medium text-neutral-700">Judul Penelitian/Proyek Terkait</label>
                        <input type="text" id="judul_penelitian" name="judul_penelitian" value="{{ old('judul_penelitian') }}" class="mt-1 block w-full px-4 py-2.5 border border-neutral-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">
                    </div>
                    <div>
                        <label for="link_media" class="block text-sm font-medium text-neutral-700">URL/Link ke Media (Contoh: Youtube, Jurnal, dll)</label>
                        <input type="url" id="link_media" name="link_media" value="{{ old('link_media') }}" class="mt-1 block w-full px-4 py-2.5 border border-neutral-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white p-6 rounded-xl border border-neutral-200/80 shadow-sm space-y-6">
                    <div>
                        ...
                        <select name="mahasiswa_id" id="mahasiswa_id" required class="mt-1 block w-full px-4 py-2.5 border border-neutral-300 bg-white rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500"><option value="" disabled selected>Pilih Mahasiswa</option>@foreach($mahasiswas as $mahasiswa)<option value="{{ $mahasiswa->mahasiswa_id }}" {{ old('mahasiswa_id') == $mahasiswa->mahasiswa_id ? 'selected' : '' }}>{{ $mahasiswa->nama_lengkap }} ({{ $mahasiswa->prodi->singkatan }})</option>@endforeach</select>
                    </div>
                    <div>
                        <label for="kategori_id" class="block text-sm font-medium text-neutral-700">Kategori</label>
                        <select name="kategori_id" id="kategori_id" class="mt-1 block w-full px-4 py-2.5 border border-neutral-300 bg-white rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500"><option value="">Tidak ada kategori</option>@foreach($kategoris as $kategori)<option value="{{ $kategori->kategori_id }}" {{ old('kategori_id') == $kategori->kategori_id ? 'selected' : '' }}>{{ $kategori->nama_kategori }}</option>@endforeach</select>
                    </div>
                    <div>
                        <label for="tahun_terbit" class="block text-sm font-medium text-neutral-700">Tahun Terbit</label>
                        <input type="number" id="tahun_terbit" name="tahun_terbit" value="{{ old('tahun_terbit', date('Y')) }}" required class="mt-1 block w-full px-4 py-2.5 border border-neutral-300 rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500">
                    </div>
                </div>
                 <div class="bg-white p-6 rounded-xl border border-neutral-200/80 shadow-sm">
                    <label for="gambar_cover" class="block text-sm font-medium text-neutral-700 mb-2">Gambar Cover</label>
