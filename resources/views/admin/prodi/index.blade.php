...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-neutral-500 md:ms-2">Kelola Prodi</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex justify-between items-center mt-4 mb-6">
        <h1 class="text-2xl font-bold text-neutral-800">Daftar Program Studi</h1>
        <a href="{{ route('admin.prodi.create') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-medium text-white bg-primary-600 border border-transparent rounded-lg shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
            ...
            <table class="w-full text-sm text-left text-neutral-600">
                <thead class="text-xs text-neutral-700 uppercase bg-neutral-100/80">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 font-semibold">Singkatan</th>
                        <th scope="col" class="px-6 py-3 font-semibold text-center">Jumlah Mahasiswa</th>
                ...
                <tbody class="divide-y divide-neutral-200">
                    @forelse ($prodis as $prodi)
                        <tr class="hover:bg-neutral-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 font-medium text-neutral-900 whitespace-nowrap">{{ $prodi->nama_prodi }}</td>
                            <td class="px-6 py-4">{{ $prodi->singkatan }}</td>
                            <td class="px-6 py-4 text-center">{{ $prodi->mahasiswas_count }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center items-center space-x-4">
                                    <a href="{{ route('admin.prodi.edit', $prodi) }}" title="Edit" class="text-primary-600 hover:text-primary-800 transition-colors">
                                    ...
                                    <form action="{{ route('admin.prodi.destroy', $prodi) }}" method="POST" class="inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" data-delete-form 
                                                data-delete-message="Apakah Anda yakin ingin menghapus prodi <strong>{{ $prodi->nama_prodi }}</strong>? Aksi ini akan berpengaruh pada data mahasiswa terkait."
